  ...
  public String createEnterpriseUser(EnterpriseRegisterRequest request) {
    try {
      ...
      if (!existingUsers.isEmpty()) {
        throw new RuntimeException("Usuário já existe no Keycloak com o e-mail: " + request.email());
      }
  ...
  public String createCandidateUser(CandidateRegisterRequest request) {
    try {
      ...
      if (response.getStatus() != 201) {
        throw new RuntimeException("Falha ao criar usuário no Keycloak: " + response.getStatusInfo());
      }
      ...
      throw new RuntimeException("Failed to create candidate user", e);
    }
  }
